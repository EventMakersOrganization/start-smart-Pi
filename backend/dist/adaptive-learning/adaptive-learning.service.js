...
let AdaptiveLearningService = AdaptiveLearningService_1 = class AdaptiveLearningService {
    ...
    async findPerformanceById(id) {
        const performance = await this.performanceModel.findById(id).exec();
        if (!performance)
            throw new common_1.NotFoundException(`Performance ${id} not found`);
        return performance;
    }
    ...
    async searchChromaChunks(query, nResults = 3) {
        try {
            const { data } = await axios_1.default.post(`${this.aiServiceBaseUrl}/search-chunks`, {
                query,
                n_results: nResults,
            ...
            this.logger.warn(`Chroma search unavailable (${query}): ${error?.message || "unknown error"}`);
            return [];
        }
    }
    async generateRecommendationsV2(studentId) {
        ...
        for (const diag of topicDiagnostics.slice(0, 3)) {
            ...
            semanticMatches.push({
                ...
                topSimilarity: typeof topSemantic?.similarity === "number"
                    ? topSemantic.similarity
                    : undefined,
            });
            const semanticCourse = String(topSemantic?.course_title || topSemantic?.course_id || "").trim();
            const semanticSnippet = String(topSemantic?.snippet || topSemantic?.chunk_text || "")
                .replace(/\s+/g, " ")
                .trim()
                .slice(0, 180);
            ...
            const payload = {
                ...
                semanticSimilarity: typeof topSemantic?.similarity === "number"
                    ? topSemantic.similarity
                    : null,
                isViewed: false,
                generatedAt: new Date(),
    ...
    async findLatestCompletedLevelTestByStudent(studentId) {
...
exports.AdaptiveLearningService = AdaptiveLearningService = AdaptiveLearningService_1 = __decorate([
